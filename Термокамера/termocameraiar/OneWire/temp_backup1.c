...
void search(unsigned char *line_mask, unsigned char id[8])
{
  ...
  if (diff != LAST_DEVICE)
  {
    diff = w1_rom_search( diff, id , *line_mask);
    if( diff == PRESENCE_ERR )
    {
      //uputsnl( "No Sensor found" );
      //ramb[0x30] = 0;
...
unsigned int search_free(unsigned char line_mask)
{
  ...
  while (adr < adr_end)
    {
      if (((hrem[adr] & 0xff00) != 0x1000)\
             && ((hrem[adr] & 0xff00) != 0x2800)) return adr;
      adr += 4;
    }
  return 0;//все €чейки пам€ти заполнены
}

unsigned char comp(unsigned char id[8], unsigned char adr_s, unsigned char adr_end)
{
  unsigned char i, c;
  unsigned int tmp;
  while (adr_s < adr_end)
