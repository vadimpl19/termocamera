...
char b;
  redon3
  b=UDR0;
  if((u0.state==sync)||((UCSR0A&0x10)!=0)){TCNT0=0;u0.state=sync;goto erx;};//была проверка на корректность кадра -стоп бит и  паритет
...
char fc16handl0(){ //функция обработки fc=16 возвращает 0 или Exeption
char i;
UINT rg;
  ...
  if (rg>=rammin){
    ...
    for(i=0;i<u0.rbuf[5];i++){ //запись в базу данных RAM
      __disable_interrupt();
...
//  _clis
//  if (maxc!=0){SREG=sregt; return 6;};//slave device busy уже идет обработка записи в eeprom c модбаса
//  if 
...
char b;
  ...
  b=UDR0;
  UCSR0B|=(1<<RXCIE0); //разрешили прерывания от приемника};
//  TIMSK_Bit1=0; //!!!! запрет прерывания от таймера
  return b;
}  
////////////////////////////////////////////////////////////////////////////////////////////////////
...
char *ptr;
  ...
  TIMSK_Bit1=0; //!!!! запрещаем прерывания от таймера OCIE0=0
